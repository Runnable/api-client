...
User.prototype.setLogger = function (log) {
  this.log = log;
};
...
User.prototype.logInfo = function (logValue, message) {
  if (!this.log) { return; }
...
User.prototype._fetchMasterInstanceByHostname = function (hostname, isolated, cb) {
  var self = this;
  function fetchInstancesAsync(query) {
    self.logInfo({
       query: query
    }, 'fetchInstancesAsync');
    return Promise.fromCallback(function (cb) {
      var instances = self.fetchInstances(query, function (err, raw) {
        self.logInfo({
          query: query,
          err: err,
          instances: instances,
  ...
  var promiseProps = {
    master: fetchInstancesAsync({
      hostname: hostname,
      masterPod: true
    })
  };
  if (isolated) {
    promiseProps.isolated = fetchInstancesAsync({
      hostname: hostname,
      isolated: isolated
    });
  }
...
User.prototype._fetchInstanceAndDepWithHostname = function (query, hostname, cb) {
  ...
  var instances = this.fetchInstances(query, function (err, raw) {
    self.logInfo({
      query: query,
      err: err,
      instances: instances,
      raw: raw
    }, '_fetchInstanceAndDepWithHostname');
    if (err) { return cb(err); }
    var instance = instances.models[0];
    if (!instance) {
    ...
    var deps = instance.fetchDependencies(query2, function (err, raw) {
      self.logInfo({
        query2: query2,
        err: err,
        depsAttrs: deps.models[0].attrs,
        raw: raw
      }, 'deps');

...
User.prototype.getConnectSid = function () {
  var jarStore = this.client.opts.jar._jar.store.idx;
  return jarStore[Object.keys(jarStore)[0]]['/']['connect.sid'].value;
};
 ...
 * @param  {String}   localIp       local ip address for the container making
 *                                  the request.
 * @param  {Function} cb            callback(err, networkIp)
 */
User.prototype.fetchInternalIpForHostname = function (hostname, localIp, cb) {
  var self = this;
  this._fetchMasterInstanceByHostname(hostname, function (err, masterInstance) {
    if (err) {
    ...
    var refererQuery = {
      ...
      'container.dockerHost': masterInstance.attrs.container.dockerHost
    };
 ...
 *                        will always exist
 */
User.prototype.fetchBackendForUrl = function (url, referer, cb) {
  var self = this;
  var parsedUrl = parseUrl(url);
  var urlPort = parsedUrl.port;
  if (!urlPort) {
    // 500 error bc developer is providing url to this function
    var err = Boom.create(500, 'url port is required (even 80)', {
      url: url,
      referer: referer
    });
    return cb(err);
 ...
 * @param  {String} cb             callback(err, dep) // can callback null dep
 */
